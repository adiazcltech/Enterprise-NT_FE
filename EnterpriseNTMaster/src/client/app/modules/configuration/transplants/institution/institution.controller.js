(function () {
    ...
    InstitutionController.$inject = [ 'institutionDS', 'localStorageService', 'logger', '$filter', '$state', 'moment', '$rootScope','LZString','$translate'
    ];
    function InstitutionController(institutionDS, localStorageService, logger,
        $filter, $state, moment, $rootScope, LZString, $translate) {
        var vm = this;
        $rootScope.menu = true;
        $rootScope.blockView = true;
        vm.init = init;
        vm.title = 'institution';
        ...
        function insert() {
          auth = localStorageService.get('Enterprise_NT.authorizationData');
          return institutionDS.New(auth.authToken, vm.Detail).then(function (data) {
            if (data.status === 200) {
              vm.get();
              vm.Detail = data.data;
        ...
        function update() {
          ...
          return institutionDS.update(auth.authToken, vm.Detail).then(function (data) {
            if (data.status === 200) {
              vm.get();
              logger.success($filter('translate')('0042'));
        ...
        function get() {
          auth = localStorageService.get('Enterprise_NT.authorizationData');
          return institutionDS.get(auth.authToken).then(function (data) {
        ...
        function getId(id, index, Form) {
          ...
          Form.$setUntouched();
          vm.loadingdata = true;
          return institutionDS.getId(auth.authToken, id).then(function (data) {
            if (data.status === 200) {
        ...
        function generateFile() {
          if (vm.filtered.length === 0) {
            ...
            vm.pathreport = '/report/configuration/transplants/institution/institution.mrt';
            vm.openreport = false;
            vm.report = false;
            vm.windowOpenReport();
          }
