(function () {
    ...
    InsuranceController.$inject = [ 'insuranceDS', 'localStorageService', 'logger', '$filter', '$state', 'moment', '$rootScope','LZString','$translate'
    ];
    function InsuranceController(insuranceDS, localStorageService, logger,
        ...
        function insert() {
          auth = localStorageService.get('Enterprise_NT.authorizationData');
          return insuranceDS.New(auth.authToken, vm.Detail).then(function (data) {
            if (data.status === 200) {
              vm.get();
              vm.Detail = data.data;
        ...
        function update() {
          ...
          return insuranceDS.update(auth.authToken, vm.Detail).then(function (data) {
            if (data.status === 200) {
              vm.get();
              logger.success($filter('translate')('0042'));
        ...
        function get() {
          auth = localStorageService.get('Enterprise_NT.authorizationData');
          return insuranceDS.get(auth.authToken).then(function (data) {
        ...
        function getId(id, index, Form) {
          ...
          Form.$setUntouched();
          vm.loadingdata = true;
          return insuranceDS.getId(auth.authToken, id).then(function (data) {
            if (data.status === 200) {
        ...
        function generateFile() {
          if (vm.filtered.length === 0) {
            ...
            vm.pathreport = '/report/configuration/transplants/insurance/insurance.mrt';
            vm.openreport = false;
            vm.report = false;
            vm.windowOpenReport();
          }
