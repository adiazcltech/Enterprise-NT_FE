...
(function () {
  ...
  angular
    .module('app.widgets')
    .directive('reportzip', reportzip);

    reportzip.$inject = ['$filter', 'localStorageService', 'common', 'orderDS', 'logger', 'generalfunctionDS', '$q', 'configurationDS'];

  /* @ngInject */
  function reportzip($filter, localStorageService, common, orderDS, logger, generalfunctionDS, $q, configurationDS) {
    var directive = {
      restrict: 'EA',
      templateUrl: 'app/widgets/report-zip.html',
      scope: {
      ...
      controller: ['$scope', function ($scope) {
        ...
        vm.downloadlistorder = downloadlistorder;
        vm.listreportEmailPatient = [];
        vm.showmodalprogressbar = showmodalprogressbar;
        ...
        $scope.$watch('openreport', function () {
          if ($scope.openreport) {
            if(vm.orders.length > 0){
              ...
              vm.listreportEmailPatient = [];
              vm.showmodalprogressbar();
              vm.getOrderend()

              $scope.openreport = false;
            }
            else{
              logger.warning("Ninguna orden es candidata para la generacion de reporte")
            }
        ...
        function getOrderend() {
          ...
          var json = {
            'rangeType': 1,
            'init': vm.orders[vm.countlistorder].order,
            'end': vm.orders[vm.countlistorder].order
          };
          vm.order = vm.orders[vm.countlistorder].order
            ...
            if (data.data.length > 0) {
              ...
              return orderDS.getOrderPreliminaryend(auth.authToken, data).then(function (data) {
                if (data.data !== '') {
                  ...
                  vm.countlistorder = vm.countlistorder + 1;
                  if(vm.orders.length === vm.countlistorder){
                    vm.downloadlistorder();
                  }
                  else{
        ...
        function sendbuffer(buffer) {
          ...
          var listreport = {
            'nameReport': vm.namePdfConfig === '1' ? vm.datareport.patientHistory : patienthistory + "_" + vm.dataorder.orderNumber,
            'buffer': byteString
          }

          vm.listreportEmailPatient.push(listreport);
          vm.changeStatePrint();

        }

        function downloadlistorder(){
          if(vm.listreportEmailPatient.length > 0){

            var promise = createreportExcel();
            promise.then(function (buffer) {
                ...
                vm.listreportEmailPatient.push(listreport)
                generalfunctionDS.saveReportPdfAll(vm.listreportEmailPatient);
                
                $('#progressbarorders9999999999').modal('toggle') 
            })
          }
          else{
            logger.warning("Ninguna orden es candidata para la generacion de reporte")
        ...
        function changeStatePrint() {
          ...
          for (var i = 0; i < vm.datareport.order.resultTest.length; i++) {
            ...
            vm.datareport.order.resultTest[i].microbiologyGrowth = {};
          }
        ...
        function createreportExcel() {
            return $q(function (resolve, reject) {
              setTimeout(function () {
                var parameterReport = {};
                var auth = localStorageService.get('Enterprise_NT.authorizationData');
                parameterReport.datareport = vm.listreportXlsx
                parameterReport.variables = {
                  "entity": "CLTECH",
                  "abbreviation": "CLT",
                  "date": moment().format(vm.formatDate + " hh:mm:ss a."),
                ...
                parameterReport.pathreport = '/reports/recordOflOrdersXLSX/reports.mrt';
                parameterReport.labelsreport = labelsreport;
                var labelsreport = null;
        ...
        function showmodalprogressbar(){
          $('#progressbarorders9999999999').modal({show: true, backdrop: 'static', keyboard: false})
        }

        function Demographicskey() {
          if (vm.urlLIS !== '' && vm.urlLIS !== null) {
            var auth = localStorageService.get('Enterprise_NT.authorizationData');
            return configurationDS.getDemographicskey(auth.authToken, vm.urlLIS, 'GenerarPDFCon').then(function (data) {
              if (data.status === 200) {
                vm.namePdfConfig = data.data.value;
