...
const processTestState = (data, state) => pkglodash.chain(data)
    .map(item => ({
        ...item,
        tests: pkglodash.filter(item.tests, test => test.result.state === state)
    }))
    .filter(item => item.tests.length > 0)
    .value();

// Refactored report rendering
dispatcher.onPost("/api/renderReport", (req, res) => {
    console
    const worker = new Worker('./reportWorker.js', {
        workerData: {
            reqBody: req.body,
            path: process.env.PATH_LIS,
            licenseKey: process.env.STIMULSOFT_LICENSE
        }
    });

    worker.on('message', (message) => {
        if (message.error) {
            console.error("Error rendering report:", message.error);
...
dispatcher.onPost("/api/generarpdf", async (req, res) => {
    try {
        ...
        console.log("tamaño de data.data", data.data.length)
        if (data.type == "pdf") {
            ...
            // Opciones específicas según la plantilla
            const options = data.template === "reviewofresult/billing"
                ? { ...commonOptions, height: "17.0in", width: "22.0in" } // Opciones para "reviewofresult/billing"
                : { ...commonOptions, format: data.format, orientation: data.orientation }; // Opciones para otras plantillas
            var document = {
            ...
            writeFile(final, output, { encoding: 'utf-8' }, function (err) {
                ...
                readFile(final, function (err, data) {
                    ...
                    // Convertir el objeto Buffer a una c adena base64
